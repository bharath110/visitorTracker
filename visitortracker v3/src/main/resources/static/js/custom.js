$(document).ready(function() {
	...
	$('#checkedInVisitorsTable').DataTable();
	$('#checkedOutVisitorsTable').DataTable();
...
function search(obj){
	...
	$('tr').each(function(){
		var row = $(this)
		var rowHide = true;
		if($(row).text().toLowerCase().indexOf($this.val().toLowerCase().trim())>-1){
			$(row).show()
		}else{
			$(row).hide()
		}
				...
				success : function(data) {
					...
					if (data.StatusCode == 200) {
						...
						if (data.carriedItems.length > 0) {
							headerOutput += "<center><h4 class=\"modal-title\">Visitor : "
									+ visitorName
									+ "</h4><\center>"
